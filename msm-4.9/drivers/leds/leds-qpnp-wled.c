...
bool g_asus_wled_auto_calib = false;
...
struct qpnp_wled {
	...
	struct work_struct	asus_shutdown_work;
	struct mutex		lock;
	...
	u16			asus_auto_calibration_ovp_count;
	u16			max_strings;
	...
	ktime_t			asus_start_ovp_fault_time;
};

extern int asus_wled_get_prop_batt_capacity(int *temp);
...
//add for ovp power off+++
/* sysfs show function for ovp count */
static ssize_t qpnp_wled_asus_ovp_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct qpnp_wled *wled = dev_get_drvdata(dev);
...
/* sysfs store function for ramp delay in each step */
static ssize_t qpnp_wled_asus_ovp_count_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct qpnp_wled *wled = dev_get_drvdata(dev);
	int data, rc;
	...
	wled->asus_ovp_count = data;
	dev_err(&wled->pdev->dev, "[WLED] asus_ovp_count=%d\n", wled->asus_ovp_count);
	return count;
}
//add for ovp power off---
...
static struct device_attribute qpnp_wled_attrs[] = {
	...
	__ATTR(asus_ovp_count, 0664, qpnp_wled_asus_ovp_count_show, qpnp_wled_asus_ovp_count_store),
...
static void asus_wled_ovp_shutdown_work(struct work_struct *work)
{
    int bat_capacity = 0;
    asus_wled_get_prop_batt_capacity(&bat_capacity);
    if (bat_capacity <= 5) {
        printk("[WLED]after wled auto calib continue ovp at %%%d soc,force shutdown device!!\n", bat_capacity);
        ASUSEvtlog("[WLED]after wled auto calib continue ovp at %%%d soc,force shutdown device!!\n", bat_capacity);
        kernel_power_off();
    } else {
        printk("[WLED] continue ovp at %%%d soc after wled auto calib\n", bat_capacity);
        ASUSEvtlog("[WLED] continue ovp at %%%d soc after wled auto calib\n", bat_capacity);
    }
}
...
//add for ovp power off+++
static bool asus_qpnp_wled_auto_cal_required(struct qpnp_wled *wled)
{
	s64 asus_elapsed_time_us;
	int ASUS_WLED_AUTO_CAL_OVP_COUNT = 5 * wled->asus_ovp_count;
	int ASUS_WLED_AUTO_CAL_CNT_DLY_US = 1000000 * wled->asus_ovp_count;	/* g_asus_wled_auto_calib_ovp_time second */
	/*
	 * Check if the OVP fault was an occasional one
	 * or if its firing continuously, the latter qualifies
	 * for an auto-calibration check.
	 */
	if (!wled->asus_auto_calibration_ovp_count) {
		wled->asus_start_ovp_fault_time = ktime_get();
		wled->asus_auto_calibration_ovp_count++;
	} else {
		asus_elapsed_time_us = ktime_us_delta(ktime_get(),
				wled->asus_start_ovp_fault_time);
		if (asus_elapsed_time_us > ASUS_WLED_AUTO_CAL_CNT_DLY_US)
			wled->asus_auto_calibration_ovp_count = 0;
		else
			wled->asus_auto_calibration_ovp_count++;

		if (wled->asus_auto_calibration_ovp_count >=
				ASUS_WLED_AUTO_CAL_OVP_COUNT) {
			wled->asus_auto_calibration_ovp_count = 0;
			return true;
		}
	}
...
static irqreturn_t qpnp_wled_ovp_irq_handler(int irq, void *_wled)
{
	...
	if((fault_sts & QPNP_WLED_OVP_FAULT_BIT)  &&
			...
			g_asus_wled_auto_calib){
		if(asus_qpnp_wled_auto_cal_required(wled)){
			schedule_work(&wled->asus_shutdown_work);
		}
	//add for ovp power off---
	}
	if (fault_sts & QPNP_WLED_OVP_FAULT_BIT) {
...
static int qpnp_wled_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&wled->asus_shutdown_work, asus_wled_ovp_shutdown_work);
	wled->ramp_ms = QPNP_WLED_RAMP_DLY_MS;
	wled->asus_ovp_count = 1;
	wled->ramp_step = 1;
