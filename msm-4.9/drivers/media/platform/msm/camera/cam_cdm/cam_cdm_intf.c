...
int cam_cdm_read_debug_register(uint32_t handle)
{
	uint32_t hw_index;
	int rc = -EINVAL;
	struct cam_hw_intf *hw = NULL;
	if (get_cdm_mgr_refcount()) {
		CAM_ERR(CAM_CDM, "CDM intf mgr get refcount failed");
		rc = -EPERM;
		return rc;
	...
	if (hw && hw->hw_ops.process_cmd) {
		rc = hw->hw_ops.process_cmd(hw->hw_priv,
				CAM_CDM_HW_INTF_CMD_PRINT_DEBUG_INFO, &handle,
				sizeof(handle));
		if (rc < 0)
			CAM_ERR(CAM_CDM,
					"failed to print debug information =%x",
					handle);
	} else {
		CAM_ERR(CAM_CDM, "hw idx %d  don't have this option",
				hw_index);
	}
	put_cdm_mgr_refcount();
	return rc;
}
EXPORT_SYMBOL(cam_cdm_read_debug_register);
