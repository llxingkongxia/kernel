	...
	enum cam_cdm_cb_status status, uint64_t cookie)
{
	...
	if (status == CAM_CDM_CB_STATUS_BL_SUCCESS) {
		...
		if(ctx->print_completion_variable) {
			CAM_INFO(CAM_ISP,
					"Called by CDM hdl=%x, udata=%pK, status=%d, cookie=%llu",
					handle, userdata, status, cookie);
			ctx->print_completion_variable = 0;
		}
		CAM_DBG(CAM_ISP,
			"Called by CDM hdl=%x, udata=%pK, status=%d, cookie=%llu",
			 handle, userdata, status, cookie);
					...
					void *acquire_hw_args)
{
	...
	ife_ctx->print_completion_variable = 1;
		...
		if (cfg->init_packet == 1) {
			...
			if (rc <= 0) {
				CAM_ERR(CAM_ISP,
					...
					cfg->request_id, rc);
				if (rc == 0) {
					cam_cdm_read_debug_register(ctx->cdm_handle);
					rc = -ETIMEDOUT;
				}
			} else {
				rc = 0;
...
int cam_ife_hw_mgr_init(struct cam_hw_mgr_intf *hw_mgr_intf)
{
	...
	for (i = 0; i < CAM_CTX_MAX; i++) {
		...
		g_ife_hw_mgr.ctx_pool[i].print_completion_variable = 0;
