...
static const struct mmc_fixup sd_fixups[] =
{
	MMC_FIXUP("USDU1", CID_MANFID_TRANSCEND, 0X4A45, add_quirk,
		  MMC_QUIRK_BLK_NO_CMD18_25),
	END_FIXUP
};
			       ...
			       struct mmc_queue *mq)
{
	...
	if (brq->data.blocks > 1) {
		...
		if(card->quirks & MMC_QUIRK_BLK_NO_CMD18_25){
			brq->data.blocks = 1;
		}
	}
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	mmc_fixup_device(card, sd_fixups);
