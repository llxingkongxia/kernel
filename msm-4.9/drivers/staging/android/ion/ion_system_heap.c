...
struct ion_system_heap *heap_system;
...
unsigned long ion_system_heap_cached_pool(void) {
	...
	if(!heap_system) {
		pr_err("ion system heap isn't created\n");
		return 0;
	}
	
	for (i = 0; i < num_orders; i++) {
		pool = heap_system->uncached_pools[i];
	...
	for (i = 0; i < num_orders; i++) {
		pool = heap_system->cached_pools[i];
	...
	for (i = 0; i < num_orders; i++) {
		for (j = 0; j < VMID_LAST; j++) {
			if (!is_secure_vmid_valid(j))
				continue;
			pool = heap_system->secure_pools[j][i];
...
EXPORT_SYMBOL(ion_system_heap_cached_pool);

unsigned long ion_system_heap_used(void){
	if(!heap_system) {
		pr_err("ion system heap isn't created\n");
		return 0;
	}
	return ion_heap_used(&heap_system->heap);
}

EXPORT_SYMBOL(ion_system_heap_used);
