...
int asus_wled_get_prop_batt_capacity(int *capacity)
{
    union power_supply_propval capacity_val = {0, };
    int rc;
    ...
    if (chip_dev == NULL) {
        printk("[WLED] chip_dev not ready yet\n");
        return -1;
    }
...
static int asus_print_all(void)
{
	...
	printk(KERN_INFO "[BATT]hvdcp_puls_cnt[135B] =0x%x, adp_allow_cfg[1360] =0x%x, usbin_opt1_cfg[1362] =0x%x, usbin_load_cfg[1365] =0x%x, usbin_icl_opt[1366] = 0x%x\n",
		value[0],
		value[1],
		value[2],
		value[3],
